...
public:
	...
	// BookRecord *getNext();				// Gets the node next (DEPRECATED)
	// void setNext(BookRecord *next);		// Sets the node next (DEPRECATED_)
    BookRecord* getLeft();               // Gets the left node
    BookRecord* getRight();               // Gets the right node
    void setLeft(BookRecord* left);     // sets the left pointer.
    void setRight(BookRecord* right);    // Sets the right pointer

    void printRecord();                 // Prints the record.
};
